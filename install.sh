...
# Configure the tools `pixi global sync` just installed.
#
# The steps live in post-sync.sh rather than here because install.sh is not the
# only thing that reaches this point: the sync skill runs its own
# init/apply/pixi sequence and needs the same tail. Keeping one copy is what
# stops a step added for install.sh from silently never running on a sync --
# which is exactly how DOTFILES_URL stayed unset on a fully synced machine.
# See that file for why neither step can be a chezmoi run script.
#
# Resolved through `chezmoi source-path`, not "$(dirname "$0")": the clone
# branch above can replace the directory this script was invoked from, so the
# applied source is the one that reliably holds a post-sync.sh matching the
# configuration just applied. Falls back to this script's own directory for the
# case where chezmoi cannot answer.
POST_SYNC="$(chezmoi source-path 2>/dev/null)/post-sync.sh"
[[ -f "$POST_SYNC" ]] || POST_SYNC="$(dirname "${BASH_SOURCE[0]}")/post-sync.sh"
if [[ -f "$POST_SYNC" ]]; then
    bash "$POST_SYNC" || warning "Post-sync configuration reported a problem — see above."
else
    warning "post-sync.sh not found — devpod's DOTFILES_URL and wf's skills are unconfigured."
fi
