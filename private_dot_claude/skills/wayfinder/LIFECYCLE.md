...
3. **Gate failure**: feed the failure output to **one** fresh retry subagent. A second failure parks the node — a `### handoff` comment on its ticket with the failure attached — and the run moves on to other work.
4. **Merging stays human.** An unattended lifecycle ends at *in review, approved*; the manager's last act on a node is a ticket comment saying so. A steering prompt may explicitly grant "merge when green" — only then squash-merge and close the ticket.
5. **Journaling**: the manager posts a `**breadcrumb:**` on the ticket at each stage transition and gate result, so a crashed lifecycle re-enters by trail exactly like any claimed ticket.
