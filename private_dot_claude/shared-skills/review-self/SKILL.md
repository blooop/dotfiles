...
reports, ranks, briefs agents, cherry-picks their commits, and writes the final
report. Everything else happens in a subagent with a fresh context and a short
return.
...
concurrently. Mutant runs in its own worktree (core §6a), so its edit never
...
that no longer exists, so the agent fixes it again on the new base instead.

Either way the agent confirms before it returns the new SHA: its test passes on
the new base, and fails with the fix's production hunk reverted. The second run
is the one that matters — a resolution can keep the test and lose the guard, or
the other fix can already make the test pass. Its return says whether it
resolved or redid. The parent cherry-picks the new SHA, which is now clean, and
...
finding, no polling (core, **How to run it**). It returns ≤150 words: the test it wrote and saw fail, the commit SHA,
...
All five reports land as commits, but not the same way. A **Defects** finding
...
A surviving mutant (core §6a) is a Tests finding of the same kind: the commit
strengthens that test, and the proof is that it now fails on the mutant Mutant
reported and passes on this branch.
...
Each fix agent runs the test it wrote and the test target it changed — not the
package. Its commit hook lints what it changed, so it runs no separate lint
step. The whole branch gets one gate after **Merge base** below: lint, the tests
that cover the area, and the build. That gate is the first time the fixes meet
each other and `main`, so it runs every time.
...
## Report, then CI only when asked

Write the **Report** below as soon as the push is done. The review ends there,
unless the invocation asks for CI; then the report waits for the CI result.

Watch CI only when the invocation says so — `/review-self and watch CI`, `until
green`, or the user asking for it in as many words. Across 28 runs, the wait for
CI was ~40% of all review time, and in most of that wait nothing ran. Without
that request, the report names the CI state at push time and says CI was not
watched, and `/pr --watch` is the command that finishes the job.

No PR yet means no CI; skip this.

When asked: a red check on your own PR is your defect, and it needs no further
proof — the run *is* the failing test core §2 asks for.
...
five axes found each defect, the mutant and whether it was killed, name the Types findings you applied and any you escalated,
say what Tests added — the cases you wrote and any coverage gap you left open — and
say plainly whether the Spec axis ran at all or reported no spec available. Then the merge — clean, or what conflicted and how it was resolved — each fix whose
cherry-pick conflicted, and whether its agent resolved it or redid it, and the CI state — watched to green,
still red and whose, or not watched, with the state at push time.
