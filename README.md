...
- **container** — devcontainers/DevPod: the eight-env floor + nvim and its config via `editor` + the full `~/.config` tree and pixi manifest (both container-local). No toolbox — no zellij or launchers *from here*, because the only things otherwise run in a workspace are `claude` and `gh` (devlaunch 0.15.0+ skips its own zellij install by default; 0.4.0 through 0.14.0 need [`DEVLAUNCH_NO_ZELLIJ`](#dev-containers-dl--aid), which `.bash_env` still exports). Identity kept, but `~/.gitconfig` is skipped in favor of the XDG fallback because DevPod overwrites it with credential injection, and `~/.claude` is skipped because `dl` bind-mounts the host's.
...
**`DEVLAUNCH_NO_ZELLIJ=1`** is exported from `.bash_env`, so every `dl`/`aid`/`wf` launch carries it. Devlaunch's setup pass installs zellij into a workspace when it finds none; these dotfiles deliberately no longer ship one there (zellij is behind `toolbox`), so without the knob devlaunch would put it back on every create and that part of the saving would evaporate. Nothing in a workspace multiplexes anything — `dl` attaches a single shell, and the multiplexer that matters is the host's, outside the container. Devlaunch read the variable from 0.4.0 and **retired it in 0.15.0** ([devlaunch#425](https://github.com/blooop/devlaunch/pull/425)), which turned the zellij stage opt-in: skip is the default there and `DEVLAUNCH_ZELLIJ=1` is the only thing that asks for the install, so on 0.15.0 and later this export is read by nothing and asks for what the default already does. It stays because `dl` on this host is 0.14.0, which still reads it; delete it once this host is on 0.15.0, and note that wanting zellij back is `DEVLAUNCH_ZELLIJ=1` rather than the absence of this line. Builds older than 0.4.0 ignore it and install their zellij into `~/.devlaunch/pixi`, which the manifest above neither installs nor syncs away. Note the export lands in a shell only after `.bash_env` is re-sourced — a terminal older than the apply still launches workspaces without it.
