...
Three ways in, split on **who decides** and **how much fog**: `/wf` when the fog needs *you* in the loop (planning only — tickets settle questions, then it hands off); `/wf-auto` when there's fog but you'd rather not be asked (principles in priority order — maintainability → simplicity → constructive modeling → test-first, extendable per map in its `## Notes` — with every resolution marked `**agent-decided:**` / *(agent)*); `/wf-one` when there's no fog at all and you just want the work tracked, resumable and gated. All three share the same tracker mechanics and the same `/wf-tdd` → gate → fresh-context `/wf-review` lifecycle, and all three park with a `### handoff` rather than guess when a call needs human hands, contradicts a recorded decision, redraws a destination you wrote, or would merge.
...
| *type, then `enter`* | On the launch line: `auto` routes to `/wf-auto` instead, and any other text is a steering prompt. `esc` backs out. |
...
**`DEVLAUNCH_NO_ZELLIJ=1`** is exported from `.bash_env`, so every `dl`/`aid`/`wf` launch carries it. Devlaunch's setup pass installs zellij into a workspace when it finds none; these dotfiles deliberately no longer ship one there (zellij is behind `toolbox`), so without the knob devlaunch puts it back on every create and that part of the saving evaporates. Nothing in a workspace multiplexes anything — `dl` attaches a single shell, and the multiplexer that matters is the host's, outside the container.

It is **pre-positioned rather than in force**: no released devlaunch reads the variable yet. 0.3.3, the newest on `prefix.dev/blooop`, knows `DEVLAUNCH_NO_TOOLS` and `DEVLAUNCH_ZELLIJ` and not this one, so today the zellij stage still runs and every create still installs a zellij — into devlaunch's own `~/.devlaunch/pixi`, so the manifest above neither installs nor syncs it away. Setting it now is free (an unrecognised variable is ignored) and means the saving arrives with the devlaunch upgrade rather than with a second dotfiles change. Until then, "a container has no zellij" is true of what these dotfiles install and not of what ends up in the container.
